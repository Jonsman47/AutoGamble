...
class BaltopCrawlerTest {
    ...
    private static class Fake implements BaltopCrawler.Environment {
        int page, opens, clicks; boolean connected=true, obstructed; String balance="1B";
        ...
        @Override public BaltopParser.Page page(){
            ...
            return new BaltopParser.Page(page,List.of(
                    new BaltopParser.Item(1,"head",page==1?"#1 Bob":"#2 Alice",List.of("Balance: $"+balance),""),
    ...
    @Test void rescanningKnownPageRefreshesStoredBalance() {
        try(var db=new BaltopDatabase(folder.resolve("cache.json"),LoggerFactory.getLogger("test"))){
            ...
            long t=1_000_000_000L;crawler.start(t);crawler.tick(t,fake,2);
            crawler.tick(t+1,fake,2);crawler.tick(t+300_000_001L,fake,2);
            assertEquals(0,db.entryOf("bob").balance().compareTo(new java.math.BigDecimal("1000000000")));
            crawler.pause("test");fake.balance="2B";t+=1_000_000_000L;
            crawler.start(t);crawler.tick(t,fake,2);crawler.tick(t+1,fake,2);crawler.tick(t+300_000_001L,fake,2);
            assertEquals(0,db.entryOf("BOB").balance().compareTo(new java.math.BigDecimal("2000000000")));
        }
    }
