...
public final class AutoGambleSettingsScreen extends Screen {
    ...
    @Override protected void init() {
        ...
        switch (page) {
            ...
            case 4 -> {
                ...
                button("Spam Payment Warning…", left, row + 32, panel, () -> { page = 10; rebuildWidgets(); });
            }
            case 10 -> {
            ...
            case 5 -> {
                if (!draft.working.tippingPermanentlyDisabled)
                    button("Disable Tipping", left, 154, panel,
                            () -> minecraft.gui.setScreen(TippingScreens.disableInfo(this, context, draft)));
            }
            case 6 -> {
                int half = panel / 2;
            ...
            case 7 -> button("Open Minecraft Key Binds…", left, row + 42, panel,
    ...
    @Override public void extractRenderState(GuiGraphicsExtractor g, int mx, int my, float delta) {
        ...
        g.centeredText(font, "1.2.4  •  " + (draft.working.dryRunMode ? "DRY RUN — no real payments" : "REAL PAYMENTS") + "  •  Save to apply", width / 2, 30, draft.working.dryRunMode ? 0xFF99DDCC : 0xFFFFBB66);
        ...
        if (page == 5) {
            ...
            g.centeredText(font, "Winning bets do not generate a tip.", width / 2, 144, 0xFFAAAAAA);
            String tipMessage = context.tipping().get().message();
            if (!tipMessage.isEmpty())
                g.centeredText(font, font.plainSubstrByWidth(tipMessage, panel), width / 2, 180, 0xFFFFBB66);
        }
        if (page == 6) {
