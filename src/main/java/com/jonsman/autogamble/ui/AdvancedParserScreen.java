...
public final class AdvancedParserScreen extends Screen {
    ...
    @Override protected void init() {
        ...
        addRenderableWidget(Button.builder(Component.literal("Import Patterns from JSON"), b -> {
            try { draft.working.incomingPaymentPatterns = PatternConfigService.read(context.configs().path()); result = "Patterns imported into draft. Test, then Save & Done."; }
    ...
    @Override public void extractRenderState(GuiGraphicsExtractor g, int mx, int my, float delta) {
        ...
        long count = draft.working.incomingPaymentPatterns.stream().filter(p -> p.enabled).count();
        g.centeredText(font, count + " enabled patterns in draft", width / 2, 33, 0xFF99DDCC);
        g.textWithWordWrap(font, Component.literal("Edit incomingPaymentPatterns in config/autogamble.json, then import. Use verified messages and named sender / amount groups."), left, 51, panel, 0xFFBBBBBB);
