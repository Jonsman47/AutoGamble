...
public final class MoneyValues {
    ...
    public static BigDecimal parse(String text) {
        ...
        String s = text.trim();
        if (s.length() > 64 || !s.matches("(?:[0-9]+|[1-9][0-9]{0,2}(?:,[0-9]{3})+)(?:\\.[0-9]{1,2})?[kKmMbBtT]?"))
        ...
        return new BigDecimal((power == 0 ? s : s.substring(0, s.length()-1)).replace(",", ""))
                .scaleByPowerOfTen(power).stripTrailingZeros();
    }
